<x-app-layout>
                ...
                <tbody id="promptContainer">
                    @forelse($prompts as $prompt)
                        <tr class="prompt-item">
                            ...
                            <td>
                                @if ($prompt->tags)
                                <span class="pf-tag">{{ $prompt->tags }}</span>@else<span
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="pf-empty"><i class="bi bi-collection"></i>No prompts found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @else
        ...
        <div id="promptContainer">
            @forelse($prompts as $prompt)
                <div class="pf-card p-4 mb-3 prompt-item {{ $prompt->is_favorite ? 'pf-favorite' : '' }}"
                    data-id="{{ $prompt->id }}">
                    <div class="row align-items-center g-3">
                        <div class="col-md-8">
                            ...
                            <p style="font-size:13px; color:#6c757d; margin-bottom:8px; line-height:1.6;">
                                {{ Str::limit($prompt->content, 160) }}</p>
                            @if ($prompt->tags)
                            ...
                            <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                                ...
                                <form action="{{ route('prompts.favorite', $prompt) }}" method="POST">
                                    ...
                                    <button type="submit" class="btn-pf-ghost"
                                        ...
                                        <i class="bi {{ $prompt->is_favorite ? 'bi-star-fill' : 'bi-star' }}"
                                            style="{{ $prompt->is_favorite ? 'color:#f59e0b;' : '' }}"></i>
                                    </button>
                                </form>
                                <a href="{{ route('prompts.show', $prompt) }}"
                                    class="btn-pf-ghost text-decoration-none">View</a>
                                ...
                                <button
                                    onclick="copyPrompt({{ \Illuminate\Support\Js::from($prompt->content) }}, {{ $prompt->id }})"
                                    class="btn-pf-success">
            ...
            @empty
                <div class="pf-card pf-empty">
                    ...
                    <a href="{{ route('prompts.create') }}" class="btn-pf-primary text-decoration-none">Create your
                        first prompt</a>
                </div>
            @endforelse
        </div>
    ...
    <div class="modal fade" id="importModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('prompts.import') }}" method="POST" enctype="multipart/form-data"
                class="modal-content" style="border-radius:14px; border:1px solid #e9ecef;">
                @csrf
    ...
    <script>
        ...
        function showToast(msg) {
            ...
            t.innerHTML =
                `<i class="bi bi-check-circle-fill" style="color:#22c55e;"></i> ${msg} <button onclick="this.parentElement.remove()" style="margin-left:auto;background:none;border:none;cursor:pointer;color:#adb5bd;"><i class="bi bi-x"></i></button>`;
