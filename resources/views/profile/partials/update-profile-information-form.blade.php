    ...
    <form method="post" action="{{ route('profile.update') }}">
        ...
        @if($errors->profile->any())
            <div style="background:#fef2f2; border:1px solid #fecaca; border-radius:8px; padding:.75rem 1rem; font-size:13px; color:#dc2626; margin-bottom:1rem;">
                @foreach($errors->profile->all() as $error)<div>{{ $error }}</div>@endforeach
            </div>
        @endif

        <div class="mb-3">
            <label class="pf-label" for="name">Full Name</label>
            <input id="name" type="text" name="name" value="{{ old('name', $user->name) }}" class="pf-input" required autofocus>
        </div>
        ...
        <div class="mb-4">
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div style="font-size:12px; color:#d97706; margin-top:6px;">
                    Email unverified.
                    <button form="send-verification" style="background:none; border:none; color:#5b5ef4; font-size:12px; cursor:pointer; text-decoration:underline; padding:0;">Resend verification</button>
                </div>
